...
if __name__ == "__main__":
    ...
    if args.run_analyzer:
        ...
        # Setup analyzers
        analysis = PlatformAnalysis(platform=platform, experiment_ids=experiment_ids,
                                    analyzers=[ThisAnalyzer], analyzers_args=[analyzers_args], analysis_name="SSMT download laser analysis")
    ...
    if args.run_download:
        ...
        with Platform('SLURM') as platform:
            ...
            analyzers = [DownloadAnalyzer(filenames=['data_brick.json'], output_path=output_dir)]
            manager = AnalyzeManager(ids=[(workitem_id, ItemType.WORKFLOW_ITEM)],
                                     analyzers=analyzers)
            manager.analyze()